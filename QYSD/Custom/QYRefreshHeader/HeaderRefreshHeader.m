...
@implementation HeaderRefreshHeader
...
- (void)prepare {
    ...
    // 资源数据（GIF每一帧）
    NSArray *idleImages = [self getRefreshingImageArrayWithStartIndex:0 endIndex:1];
    NSArray *refreshingImages = [self getRefreshingImageArrayWithStartIndex:2 endIndex:13];
    // 普通状态
...
- (NSArray *)getRefreshingImageArrayWithStartIndex:(NSInteger)startIndex endIndex:(NSInteger)endIndex{
    ...
    for (NSUInteger i = startIndex; i <= endIndex; i++) {
        UIImage *image = [UIImage imageNamed:[NSString stringWithFormat:@"loading_h_w_%lu", (unsigned long)i]];
        if (image) {
