...
- (UIImageView *)im{
    ...
    if (!_im) {
        ...
        _im.image = [UIImage imageNamed:@"img_gussyoulike"];
...
- (void)setupTitles {
    ...
    for (NSString *title in self.titles) {
        ...
        if (index == 1) {
//            titleView.text = @"";
//            [self.im addGestureRecognizer:tapGes];
            [titleView addSubview:self.im];
        }
...
//    CGFloat titleY = 0.0;
    CGFloat titleW = 0.0;
    CGFloat titleH = self.segmentStyle.titleHeight == 0? self.zj_height - self.segmentStyle.scrollLineHeight:self.segmentStyle.titleHeight;
    CGFloat  titleY = (self.scrollView.bounds.size.height - titleH)/2;
    ...
    if (!self.segmentStyle.isScrollTitle) {// 标题不能滚动, 平分宽度
        ...
        for (ZJTitleView *titleView in self.titleViews) {
            titleW = [self.titleWidths[index] floatValue] + 20;
            ...
            index++;
            if ([titleView.text isEqualToString:@"猜你喜欢"]) {
                
                titleView.text = @"";
                self.im.frame = titleView.bounds;
            }
        }
    ...
    [UIView animateWithDuration:animatedTime animations:^{
        ...
        if (weakSelf.scrollLine) {
            if (weakSelf.segmentStyle.isScrollTitle) {
                weakSelf.scrollLine.zj_x = currentTitleView.zj_x + 10;
                weakSelf.scrollLine.zj_width = currentTitleView.zj_width - 20;
            } else {
                if (self.segmentStyle.isAdjustCoverOrLineWidth) {
...
- (void)adjustUIWithProgress:(CGFloat)progress oldIndex:(NSInteger)oldIndex
                currentIndex:(NSInteger)currentIndex {
    
    DLog(@"procee===%f",progress);
    if (oldIndex < 0 ||
    ...
    if (self.scrollLine) {
        ...
        if (self.segmentStyle.isScrollTitle) {
            self.scrollLine.zj_x = oldTitleView.zj_x + xDistance * progress + 10;
            self.scrollLine.zj_width = oldTitleView.zj_width + wDistance * progress - 20;
        } else {
            if (self.segmentStyle.isAdjustCoverOrLineWidth) {
